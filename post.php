<?php
    ...
    function addPost($text=null, $image=null)
    {
        ...
        $query="INSERT INTO `feeds`(`user`, `text`) VALUES ('$user', '$text')";
        if($image!=null)
            $query="INSERT INTO `feeds`(`user`, `text`, `images`) VALUES ('$user', '$text', '$image')";
        $res=mysqli_query($GLOBALS['conn'], $query);
        if($res!=false)
        {
            $_SESSION['first-update']=true;
    ...
    if(isset($_POST['post-text']) || isset($_FILES['upload']))
    {
        $text=null;
        $image=null;

        $text=mysqli_real_escape_string($GLOBALS['conn'], $_POST['post-text']);
