<?php 
    ...
    function showComments()
    {
        if($GLOBALS['commentRes']!=null && mysqli_num_rows($GLOBALS['commentRes'])>0)
        {
            while($row=mysqli_fetch_assoc($GLOBALS['commentRes']))
            {
                ...
                <h5 class="text-left text-dark border rounded-0" style="font-family: ABeeZee, sans-serif;padding: 10px;margin-bottom: 0px;font-size: 13px;">'.$row['comment'].'</h5>
    ...
    $id=$_GET['id'];
    $id=substr($id, 6);
    $query="SELECT * FROM `feeds` WHERE `sno`='$id'";
    $res=mysqli_query($GLOBALS['conn'], $query);
    $postRow=null;
    $commentRes=null;
    $userRow=null;
    if($res!=false && mysqli_num_rows($res)==1)
    {
        ...
        $query="SELECT * FROM `comments` WHERE `post`='$id' ORDER BY `additionTime` DESC";
...
<body style="background-position: auto;background-size: cover;background-repeat: no-repeat;background-image: url(&quot;assets/img/blur.png&quot;);">
    <div class="col-lg-4 offset-lg-4">
        <div class="card" style="margin-top: 30px;background-color: #f9f9f9;">
            <div class="card-body">
                <div class="col-xl-12 offset-xl-9 text-center" style="margin-left: 0px;padding-right: 0px;padding-left: 0px;"><a class="btn btn-outline-primary btn-sm text-right border rounded-0" role="button" style="margin: 0px;padding: 10px;padding-top: 10px;padding-bottom: 10px;padding-right: 8px;padding-left: 8px;" href="home"><i class="fa fa-feed"></i>&nbsp; Feed</a>
                    <a
                        class="btn btn-outline-primary btn-sm border rounded-0" role="button" style="padding: 10px;padding-top: 10px;padding-right: 8px;padding-bottom: 10px;padding-left: 8px;" href="logout.php"><i class="fas fa-sign-out-alt" style="margin-right: 5px;"></i>&nbsp;Log out</a>
                </div>
                ...
                <p class="text-left border rounded-0 card-text" style="padding: 10px;margin-bottom: 10px;background-color: rgba(0,0,0,0.04);font-size: 16px;"><?php echo $postRow['text']; ?></p>
                ...
                <form method="POST" action="addComment.php">
                    ...
                    <div class="col" style="margin-top: 40px">
                        <div class="form-group">
                            ...
                            <input class="form-control form-control-sm" type="text" name="comment" placeholder="Add a comment" name="comment" required="">
                        </div>
                    </div>
                    <div class="col">
                        
                        <button class="ui facebook button" type="submit" name="addComment" value="postid<?php echo $postRow['sno']; ?>">
                              Comment
                            </button>
                    </div>
                </form>
